...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/berk/Documents/Projects/language-intermodulation/test.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    word1 = visual.TextStim(win=win, name='word1',
        text='Red',
        font='Arial',
        pos=(-1, 0), height=1.0, wrapWidth=None, ori=0.0, 
    ...
    word2 = visual.TextStim(win=win, name='word2',
        text='Boat',
        font='Arial',
        units='deg', pos=(1, 0), height=1.0, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    fixation_dot = visual.ShapeStim(
        win=win, name='fixation_dot',
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if word1.status == NOT_STARTED and tThisFlip >= 1.-frameTolerance:
        ...
        if word1.status == STARTED:
            ...
            if tThisFlipGlobal > word1.tStartRefresh + 2.-frameTolerance:
        ...
        if word2.status == NOT_STARTED and tThisFlip >= 1.-frameTolerance:
            # keep track of start time/frame for later
        ...
        if word2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > word2.tStartRefresh + 2.-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if fixation_dot.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'fixation_dot.started')
            # update status
            fixation_dot.status = STARTED
            fixation_dot.setAutoDraw(True)
        
        # if fixation_dot is active this frame...
        ...
        if fixation_dot.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fixation_dot.tStartRefresh + 3.-frameTolerance:
                # keep track of stop time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fixation_dot.stopped')
                # update status
